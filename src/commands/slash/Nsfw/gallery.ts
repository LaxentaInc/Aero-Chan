...
function setUserCacheTimeout(userId: any) {
  if (userShownVideos.has(userId)) {
    const entry = userShownVideos.get(userId) as any;
...
async function getAuthToken() {
  try {
    const res = await (axios.get("https://api.redgifs.com/v2/auth/temporary") as any);
...
async function fetchMedia(tag: any) {
...
async function updateReply(interaction: any, data: any) {
  if (interaction.guild) {
    return await interaction.editReply(data).then((res: any) => res);
  } else {
 ...
 */
async function getBestMatchingTag(userInput: any) {
  const suggestions = await getTagSuggestions(userInput, 25);
  if (suggestions.length === 0) return userInput;

  // Find exact match first
  const exactMatch = suggestions.find(tag => tag.toLowerCase() === userInput.toLowerCase()) as any;
  if (exactMatch) return exactMatch;

  // Find closest match (starts with user input)
  const startsWith = suggestions.find(tag => tag.toLowerCase().startsWith(userInput.toLowerCase())) as any;
  if (startsWith) return startsWith;

  // Find contains match
  const contains = suggestions.find(tag => tag.toLowerCase().includes(userInput.toLowerCase())) as any;
 ...
 */
function selectRandomVideo(videos: any, userId: any) {
  const userEntry = userShownVideos.get(userId) as any || {
    videos: new Set()
  };

  // Filter out already shown videos
  let available = videos.filter((video: any) => !userEntry.videos.has(video.id));
 ...
 */
function getVideoCategory(video: any) {
 ...
 */
async function sendMedia(interaction: any, tag: any, isPublic: any) {
  ...
  const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`next_${baseId}`).setLabel("Next").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`details_${baseId}`).setLabel("Details").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`share_${baseId}`).setLabel("Share").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`delete_${baseId}`).setLabel("Delete").setStyle(ButtonStyle.Danger));
 ...
 */
function registerButtonHandlers(interaction: any, tag: any, message: any, isPublic: any) {
  const baseId = `${tag}_${interaction.id}`;
  const filter = (i: any) => (i.customId.includes(`next_`) || i.customId.includes(`details_`) || i.customId.includes(`share_`) || i.customId.includes(`delete_`)) && i.user.id === interaction.user.id;
  ...
  collector.on("collect", async (i: any) => {
    ...
    if (i.customId.startsWith("delete_")) {
      ...
      await i.deferUpdate();
      const video = videoCache.get(buttonBaseId) as any;
      if (video) {
        ...
        if (isPublic) {
          ...
          const warningEmbed = new EmbedBuilder().setColor(0xff0000).setTitle('Content Warning').setDescription("Umm this shared content won't be auto-embedded in here.\nSo whoever is interested- click below to view it :3\nON your own risk btw :3").setFooter({
            text: `Shared by ${interaction.user.username}`
          });
          const viewButton = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`view_${buttonBaseId}`).setLabel("View Content").setStyle(ButtonStyle.Primary));
          ...
          const viewFilter = (btn: any) => btn.customId === `view_${buttonBaseId}`;
          const viewCollector = warningMessage.createMessageComponentCollector({
            filter: viewFilter,
            time: 300000
          });
          viewCollector.on("collect", async (btn: any) => {
            try {
              await btn.reply({
                content: formattedLink,
                ephemeral: true
          ...
          viewCollector.on("end", async () => {
            const disabledViewButton = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`view_${buttonBaseId}`).setLabel("View Content").setStyle(ButtonStyle.Primary).setDisabled(true));
            try {
              await warningMessage.edit({
                components: [disabledViewButton]
              });
            } catch (err: any) {
  ...
  collector.on("end", async (_collected: any, reason: any) => {
    if (reason !== "next" && reason !== "delete") {
      const disabledButtons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("disabled_next").setLabel("Next").setStyle(ButtonStyle.Primary).setDisabled(true), new ButtonBuilder().setCustomId("disabled_details").setLabel("Details").setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId("disabled_share").setLabel("Share").setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId("disabled_delete").setLabel("Delete").setStyle(ButtonStyle.Danger).setDisabled(true));
      try {
        await updateReply(interaction, {
 ...
 */
async function getTagSuggestions(query: any, count: number = 25) {
  // Check cache first
  const cacheKey = `${query.toLowerCase()}_${count}`;
  if (tagSuggestionsCache.has(cacheKey)) {
    const cached = tagSuggestionsCache.get(cacheKey) as any;
...
export default {
  data: new SlashCommandBuilder().setName("realp").setDescription("View random curated media from RedGifs").addStringOption((option: any) => option.setName("tag").setDescription("Enter search query (use autocomplete for best results)").setRequired(true).setAutocomplete(true)).setIntegrationTypes(0, 1).setContexts(0, 1, 2),
  ...
  async autocomplete(interaction: any) {
    ...
    const choices = suggestions.filter((tag: any) => tag.toLowerCase().includes(focusedValue.toLowerCase())).slice(0, 25).map((tag: any) => ({
      name: tag,
      value: tag
    }));
