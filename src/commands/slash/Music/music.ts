...
export default {
  data: new SlashCommandBuilder().setName('music').setDescription('Premium & Advanced music player for seamless music playback with homies in VC').addSubcommand((subcommand: any) => subcommand.setName('play').setDescription('Search and play music').addStringOption((option: any) => option.setName('query').setDescription('Song name, URL, or search term').setRequired(true).setMaxLength(500)).addStringOption((option: any) => option.setName('source').setDescription('Preferred music source').setRequired(false).addChoices({
    name: 'Auto (Spotify → YouTube)',
    value: 'auto'
  }, {
    name: 'Spotify',
    value: 'spotify'
    ...
    name: 'SoundCloud',
    value: 'soundcloud'
  }))).addSubcommand((subcommand: any) => subcommand.setName('queue').setDescription('View the current music queue')).addSubcommand((subcommand: any) => subcommand.setName('nowplaying').setDescription('Show currently playing track')).addSubcommand((subcommand: any) => subcommand.setName('skip').setDescription('Skip the current track')).addSubcommand((subcommand: any) => subcommand.setName('stop').setDescription('Stop music and clear queue'))
  // .addSubcommand(subcommand =>
  //     subcommand
  //         .setName('pause')
  //         .setDescription('Pause/resume playback'))
  .addSubcommand((subcommand: any) => subcommand.setName('loop').setDescription('Toggle loop modes').addStringOption((option: any) => option.setName('mode').setDescription('Loop mode').setRequired(false).addChoices({
    name: 'Off',
    value: 'off'
  }, {
    name: 'Track',
    ...
    name: 'Queue',
    value: 'queue'
  }))).addSubcommand((subcommand: any) => subcommand.setName('clear').setDescription('Clear the queue')).addSubcommand((subcommand: any) => subcommand.setName('disconnect').setDescription('Disconnect from voice channel')).setDefaultMemberPermissions(PermissionFlagsBits.UseVoiceActivation),
...
async function handlePlay(interaction: any) {
  ...
  if (!voiceCheck.success) {
    return await interaction.editReply({
      embeds: [createSafeErrorEmbed(voiceCheck.title, voiceCheck.message)]
    });
  }
  ...
  try {
    ...
    if (isUrl) {
      ...
      if (domain) {
        const isAllowed = allowedDomains.some((d: any) => domain.includes(d));
        if (!isAllowed) {
          return await interaction.editReply({
    ...
    try {
      if (isUrl) {
        ...
        searchResult = source === 'auto' ? await interaction.client.smartSearch(query, interaction.user) : await interaction.client.searchTrack(query, interaction.user, source);
      }
    ...
    if (!player) {
      player = await interaction.client.createPlayer(interaction.guild.id, member.voice.channel.id, interaction.channel.id);
    }
    ...
    await interaction.client.displaySearchResults(interaction, searchResult.data, query, interaction.user);
  } catch (error: any) {
...
// ==================== SKIP SUBCOMMAND ====================
async function handleSkip(interaction: any) {
  const player = interaction.client.getPlayer(interaction.guild.id);
...
// ==================== LOOP SUBCOMMAND ====================
async function handleLoop(interaction: any) {
  const player = interaction.client.getPlayer(interaction.guild.id);
  const voiceCheck = await validateVoiceWithPlayer(interaction, player);
  if (!voiceCheck.success) {
    return await interaction.editReply({
...
// ==================== HELPER FUNCTIONS ====================
async function validateVoiceState(interaction: any) {
  ...
  if (!voicePermissions.has(['Connect', 'Speak'])) {
    return {
      ...
      message: `I need Connect and Speak permissions in **${memberVoice.name}**.`
    };
  }
  if (botVoice && memberVoice.id !== botVoice.id) {
    return {
      ...
      message: `I'm already in **${botVoice.name}**. Join that channel or disconnect me first.`
    };
  }
  return {
    success: true
...
async function validateVoiceWithPlayer(interaction: any, player: any) {
  if (!player) {
    return {
      success: false,
      title: 'No Active Player',
      message: 'No music session is currently active.'
    };
  }
  const member = interaction.guild.members.cache.get(interaction.user.id) as any;
...
async function handleError(interaction: any, error: any) {
  ...
  .setAuthor({
    name: '❌ Command Failed',
    iconURL: CUSTOM_ICON
  }).setDescription(errorMessage).addFields({
    name: 'Error Details',
    value: `\`\`\`${errorDetails.slice(0, 200)}\`\`\``
  } as any).setTimestamp();
  try {
