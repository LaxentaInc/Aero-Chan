...
async function fetchServerStatus(serverAddress: any) {
  try {
...
function createServerStatusMessage(data: any, serverAddress: any) {
  // Determine if the server is offline
  const isServerOffline = !data.version || typeof data.version === 'string' && data.version.toLowerCase() === 'offline';
  if (isServerOffline) {
...
function handleError(error: any, interaction: any) {
...
export default {
  data: new SlashCommandBuilder().setName('mcstatus').setDescription('Fetches the status (online/offline, players, etc.) of a Minecraft server').addStringOption((option: any) => option.setName('server').setDescription('The server address (e.g. hypixel.net)').setRequired(true)),
