...
export default {
  data: new SlashCommandBuilder().setName('roleall').setDescription('Assign or remove a role for all members in the server.').setContexts(0, 1) // 0 = Guild, 1 = User, 2 = DM (Excluded)
  .addStringOption((option: any) => option.setName('action').setDescription('Whether to add or remove the role').setRequired(true).addChoices({
    name: 'Add Role',
    value: 'add'
  }, {
    name: 'Remove Role',
    value: 'remove'
  })).addRoleOption((option: any) => option.setName('role').setDescription('The role to assign or remove').setRequired(true)),
  async execute(interaction: any) {
    ...
    const validMembers = members.filter((member: any) => !member.user.bot); // Exclude bots
    ...
    let successCount = 0;
    let failCount = 0;
    const promises = validMembers.map(async (member: any) => {
      try {
        if (action === 'add') {
