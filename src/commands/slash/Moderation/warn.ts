...
export default {
  data: new SlashCommandBuilder().setName('warn').setDescription('Warn a user and add a strike to their AntiSpam record').addUserOption((option: any) => option.setName('user').setDescription('The user to warn').setRequired(true)).addStringOption((option: any) => option.setName('reason').setDescription('Reason for the warning').setRequired(true)),
  async execute(interaction: any) {
    ...
    try {
      ...
      const embed = {
        title: result.action === 'punished' ? `<:mod:1422451081224392816> **${targetUser.tag}** Punished` : `<:mod:1422451081224392816> **${targetUser.tag}** Warned`,
        description: `**User:** <@${targetUser.id}>\n**Reason:** ${reason}\n**Strikes:** ${result.strikes}/${result.maxStrikes}`,
        color: result.action === 'punished' ? 0xFF0000 : 0xFFA500,
        footer: {
          text: result.action === 'punished' ? 'Maximum Strike Threshold reached | Action taken' : 'Strike added | AntiSpam Moderation'
        }
      };
