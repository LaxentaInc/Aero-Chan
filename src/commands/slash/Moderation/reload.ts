...
export default {
  data: new SlashCommandBuilder().setName("reload").setDescription("Reloads a specific command without restarting (developer only)").setIntegrationTypes(0, 1).setContexts(0, 1, 2).addStringOption((option: any) => option.setName("type").setDescription("Choose command type: 'slash' or 'prefix'").setRequired(true).addChoices({
    name: "Slash Command",
    value: "slash"
  }, {
    name: "Prefix Command",
    value: "prefix"
  })).addStringOption((option: any) => option.setName("command").setDescription("The command name to reload").setRequired(true)),
  async execute(interaction: any) {
    ...
    try {
      ...
      const commandCollection = type === "slash" ? interaction.client.slashCommands : interaction.client.prefixCommands;
      if (!commandCollection) {
        throw new Error(`${type} commands collection not found!`);
      }

      // Recursive function to find the command file.
      function findCommandFile(dir: any, cmdName: any) {
      ...
      if (!foundCommand) {
        function fuzzySearch(dir: any, cmdName: any) {
          const items = fs.readdirSync(dir);
          let matches = [];
          for (const item of items) {
            const fullPath = path.join(dir, item);
            ...
            } else if (item.toLowerCase().includes(cmdName.toLowerCase()) && item.endsWith(".js")) {
              matches.push({
                path: fullPath,
                relativePath: path.relative(baseDir, fullPath)
              });
        ...
        if (fuzzyMatches.length === 1) {
          ...
          const matchList = fuzzyMatches.map((cmd: any) => `\`${path.basename(cmd.path, ".js")}\``).join(", ");
      ...
      const oldCommand = type === "slash" ? commandCollection.get(commandName) as any : Array.from(commandCollection.values()).find(cmd => cmd.name === commandName) as any;
      if (oldCommand) {
        if (type === "slash") {
      ...
      if (type === "slash" && (!newCommand.data || !newCommand.execute)) {
        throw new Error("The slash command file is missing required properties ('data' or 'execute').");
      }
      if (type === "prefix" && (!newCommand.name || !newCommand.execute)) {
        throw new Error("The prefix command file is missing required properties ('name' or 'execute').");
      }
      ...
      if (type === "slash") {
        ...
        const registeredCommand = appCommands.find(cmd => cmd.name === newCommand.data.name) as any;
      ...
      console.log(`[Reload] ${interaction.user.tag} reloaded ${type}/${foundCommand.relativePath}`);
    } catch (error: any) {
