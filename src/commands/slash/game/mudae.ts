...
export default {
  data: new SlashCommandBuilder().setName('mudae').setDescription('Mudae and Economy System').setIntegrationTypes(0, 1).setContexts(0, 1, 2).addSubcommand((subcommand: any) => subcommand.setName('help').setDescription('Shows the guide for the economy and waifu system')),
  async execute(interaction: any) {
    if (interaction.options.getSubcommand() === 'help') {
      const economyDir = path.join(__dirname, '../../../commands/prefix/economy');
      const commands = [];
      try {
        const files = fs.readdirSync(economyDir).filter((file: any) => file.endsWith('.js'));
      ...
      const embed = new EmbedBuilder().setColor('#FF69B4').setTitle('🌸 Mudae & Economy Guide 🌸').setDescription(`Welcome to the new economy! It works in Globally in all servers. Here is how you can earn money, gamble, and marry your favorite characters.\n\n**Current Balance:** ${economy.formatCurrency(balance)}`).addFields({
        name: 'Earning Money',
