...
// Command Builder
const command = new SlashCommandBuilder().setName('afk').setDescription('Set your AFK status').addStringOption((option: any) => option.setName('reason').setDescription('Reason for going AFK').setRequired(false));
export default {
  data: command,
  async execute(interaction: any) {
    try {
      ...
      const cooldownExpiration = cooldowns.get(cooldownKey) as any;
      if (cooldownExpiration && Date.now() < cooldownExpiration) {
        const remainingTime = Math.ceil((cooldownExpiration - Date.now()) / 1000);
        return interaction.reply({
          embeds: [new EmbedBuilder().setColor(0xFF0000).setDescription(`⏰ Please wait ${remainingTime} seconds before using AFK again.`)],
      ...
      if (existingAfk) {
        ...
        return interaction.reply({
          embeds: [new EmbedBuilder().setColor(0x0000FF).setDescription(`<a:yay:1327982490144735253> Welcome back ${interaction.user}! You were AFK for ${timeAFK}.`)]
        });
      }
      ...
      await interaction.reply({
        embeds: [new EmbedBuilder().setColor(0x0000FF).setDescription(`<a:kittycat:1333358006720794624> ${interaction.user} is now AFK: ${reason}`).setFooter({
          text: 'Send any message anywhere in the guild to remove your AFK status :3 Cya'
        })]
      });
      ...
      interaction.reply({
        embeds: [new EmbedBuilder().setColor(0xFF0000).setDescription('<a:kittycat:1333358006720794624> Unable to set your AFK status.')],
        flags: MessageFlags.Ephemeral
      });
    }
  }
}; // Helper Functions
async function updateNickname(interaction: any) {
...
async function resetNickname(interaction: any) {
  const currentNickname = interaction.member.displayName;
  if (currentNickname.includes('[AFK]')) {
    try {
