...
function getCategoryDropdown(dropdownOptions: any, page: any, categoriesPerPage: number = 25) {
  ...
  const select = new StringSelectMenuBuilder().setCustomId("help-category-select").setPlaceholder("📚 Choose a command category...").addOptions(options);
...
function getCategoryCommandEmbed(categoryName: any, cmds: any, commandPage: any, commandsPerPage: any, client: any) {
  ...
  const embed = new EmbedBuilder().setColor("#5865F2").setAuthor({
...
async function createCommandPaginationRow(messageId: any, commandPage: any, totalPages: any, userId: any) {
  ...
  if (commandPage > 0) {
    ...
    components.push(new ButtonBuilder().setCustomId(prevId).setLabel("Previous").setStyle(ButtonStyle.Primary).setEmoji("◀️"));
  }
  if (commandPage < totalPages - 1) {
    ...
    components.push(new ButtonBuilder().setCustomId(nextId).setLabel("Next").setStyle(ButtonStyle.Primary).setEmoji("▶️"));
  }
  ...
  await registerButton(backId, [userId], async (interaction: any) => {
    ...
    const totalCommands = Object.values(state.categories).reduce((acc: any, cmds: any) => acc + cmds.length, 0);
    const mainEmbed = new EmbedBuilder().setColor("#5865F2").setAuthor({
      name: "Command Help Menu",
      iconURL: interaction.client.user.displayAvatarURL()
    }).setDescription(`Welcome to the help menu! Select a category below to explore available commands.\n\n` + `<:helppppp:1437818267489013960> **Statistics**\n` + `└ ${totalCategories} Categories\n` + `└ ${totalCommands} Total Commands\n\n` + `💡 **Tip:** Use the dropdown menu below to navigate through categories!`).setThumbnail(interaction.client.user.displayAvatarURL()).setTimestamp().setFooter({
  ...
  components.push(new ButtonBuilder().setCustomId(backId).setLabel("Back to Menu").setStyle(ButtonStyle.Secondary).setEmoji("<a:computer6:1333357940341735464>"));
  return components.length > 0 ? new ActionRowBuilder().addComponents(components) : null;
}
export default {
  data: new SlashCommandBuilder().setName("help").setContexts([0, 2]).setDescription("Lists all available slash commands by category."),
  cooldown: 30,
  async execute(interaction: any) {
    try {
      ...
      const commandFolders = fs.readdirSync(slashFolderPath).filter((file: any) => fs.statSync(path.join(slashFolderPath, file)).isDirectory());

      // Build the categories object by loading each command file.
      const categories = {};
      for (const folder of commandFolders) {
        const commandFiles = fs.readdirSync(path.join(slashFolderPath, folder)).filter((file: any) => file.endsWith(".js"));
      ...
      const totalCommands = Object.values(categories).reduce((acc: any, cmds: any) => acc + cmds.length, 0);
      const mainEmbed = new EmbedBuilder().setColor("#5865F2").setAuthor({
        name: "Command Help Menu",
        iconURL: interaction.client.user.displayAvatarURL()
      }).setDescription(`Welcome to the help menu! Select a category below to explore available commands.\n\n` + `<:helppppp:1437818267489013960> **Statistics**\n` + `└ ${totalCategories} Categories\n` + `└ ${totalCommands} Total Commands\n\n` + `💡 **Tip:** Use the dropdown menu below to navigate through categories!`).setThumbnail(interaction.client.user.displayAvatarURL()).setTimestamp().setFooter({
      ...
      collector.on("collect", async (i: any) => {
        if (i.user.id !== interaction.user.id) {
          return i.reply({
            content: "Brotha You cannot use this.",
            ephemeral: true
          });
        }
        if (i.isStringSelectMenu() && i.customId === "help-category-select") {
          ...
          const commandEmbed = getCategoryCommandEmbed(chosenCategory, cmds, state.commandPage, state.commandsPerPage, interaction.client);
          const paginationRow = await createCommandPaginationRow(replyMsg.id, state.commandPage, totalPages, interaction.user.id);
          await i.update({
            embeds: [commandEmbed],
      ...
      if (!interaction.deferred) {
        await interaction.reply({
          content: "Something went wrong while executing the help command.",
          ephemeral: true
        });
      } else {
