...
export default {
  data: new SlashCommandBuilder().setName('dashboard').setDescription('Access Aero\'s web dashboard!').setIntegrationTypes(0, 1).setContexts(0, 1, 2),
  async execute(interaction: any) {
    const DASHBOARD_URL = 'https://www.laxenta.tech/dashboard';
    const SUPPORT_SERVER = 'https://discord.gg/C9t8dQABgY';
    const embed = new EmbedBuilder().setColor('#5865F2').setTitle('<a:ehe:1310498098107387974> Aero\'s Web Dashboard').setDescription(`Control **${interaction.client.user.username}** from your browser with our powerful web dashboard!`).addFields({
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel('Open Dashboard').setStyle(ButtonStyle.Link).setURL(DASHBOARD_URL).setEmoji('<a:zzapinkheartexclam_1327982490144:1342442561297711175>'), new ButtonBuilder().setLabel('Support Server').setStyle(ButtonStyle.Link).setURL(SUPPORT_SERVER).setEmoji('<a:pats_1327965154998095973:1332327251253133383>'), new ButtonBuilder().setLabel('Guide').setStyle(ButtonStyle.Link).setURL(`${DASHBOARD_URL}/guide`).setEmoji('📖'));
