...
function getWeatherEmoji(condition: any) {
...
function getTempColor(tempC: any) {
...
class CacheManager {
  ...
  set(key: any, value: any, ttl: number = 15 * 60 * 1000) {
    this.cache.set(key, {
      value,
      expiry: Date.now() + ttl
    });
...
async function fetchCitySuggestions(query: any) {
  ...
  try {
    ...
    return response.data.map((loc: any) => `${loc.name}, ${loc.region || ''}, ${loc.country}`.replace(/, ,/g, ','));
  } catch (error: any) {
    logger.error(`City suggestion error: ${error.message}`);
    return [];
  }
}
async function fetchWeatherData(city: any, dataType: string = 'both') {
  const cacheKey = `${city}_${dataType}`;
  const cachedData = weatherCache.get(cacheKey) as any;
...
function getAQIDescription(aqi: any) {
...
function formatTemperature(temp_c: any, temp_f: any, units: string = 'both') {
  if (units === 'metric') return `${temp_c}°C`;
  if (units === 'imperial') return `${temp_f}°F`;
  return `${temp_c}°C (${temp_f}°F)`;
}
function getWindDescription(speed_kph: any) {
  const descriptions = [{
    max: 1,
    desc: "Calm",
    emoji: "🍃"
  ...
  const wind = descriptions.find(d => speed_kph < d.max) as any;
  return wind || descriptions[descriptions.length - 1];
}
function getUVDescription(uv: any) {
  if (uv <= 2) return {
    level: "Low",
    emoji: "🟢",
...
function formatLocalTime(datetime: any) {
...
function buildCurrentWeatherEmbed(state: any) {
  ...
  embed.setFooter({
    text: `Last updated: ${formatLocalTime(primaryLoc.localtime)}`
  });
  return embed;
}
function buildForecastEmbed(state: any) {
  const embed = new EmbedBuilder();
  ...
  embed.addFields({
    name: `${weatherEmoji} ${primaryLoc.name} - ${new Date(fcDay.date).toLocaleDateString('en-US', {
      weekday: 'long',
      month: 'short',
      day: 'numeric'
    })}`,
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const dayData = isNextDay && primaryData.forecast.forecastday.length > 1 ? primaryData.forecast.forecastday[1] : fcDay;
  ...
  if (primaryData.forecast.forecastday.length > 1) {
    ...
    embed.addFields({
      name: `${tomorrowEmoji} Tomorrow - ${new Date(tomorrow.date).toLocaleDateString('en-US', {
        weekday: 'long',
        month: 'short',
        day: 'numeric'
      })}`,
  ...
  embed.setFooter({
    text: 'Forecast data provided by WeatherAPI'
  });
  return embed;
}
function buildAstronomyEmbed(state: any) {
  const embed = new EmbedBuilder();
  const primaryData = state.primary.forecast;
  const compareData = state.compare?.forecast;
  if (!primaryData || !primaryData.forecast || !primaryData.forecast.forecastday.length) {
  ...
  embed.setFooter({
    text: 'All times are in local timezone'
  });
  return embed;
}
function buildAlertsEmbed(state: any) {
  const embed = new EmbedBuilder();
  const primaryData = state.primary.forecast;
  const compareData = state.compare?.forecast;
  if (!primaryData) {
  ...
  if (hasAlertsForPrimary) {
    ...
    primaryData.alerts.alert.slice(0, 3).forEach((alert: any, index: any) => {
      ...
      embed.addFields({
        ...
        value: [`**Severity:** ${alert.severity || 'Unknown'}`, `**Certainty:** ${alert.certainty || 'Unknown'}`, `**Urgency:** ${alert.urgency || 'Unknown'}`, alert.effective && alert.expires ? `**Valid:** ${formatDate(alert.effective)} - ${formatDate(alert.expires)}` : '', `\n*${truncateText(alert.desc || alert.description || 'No description available', 300)}*`].filter(Boolean).join('\n'),
        inline: false
      } as any);
    });
  ...
  if (compareData) {
    ...
    if (hasAlertsForCompare) {
      ...
      compareData.alerts.alert.slice(0, 2).forEach((alert: any, index: any) => {
        const severityEmoji = getSeverityEmoji(alert.severity);
        embed.addFields({
  ...
  embed.setFooter({
    text: 'Stay safe and monitor local weather services'
  });
  return embed;
}
function buildLocationEmbed(state: any) {
  const embed = new EmbedBuilder();
  const primaryData = state.primary.current;
  const compareData = state.compare?.current;
  if (!primaryData) {
  ...
  embed.addFields({
    ...
    value: [`**Region:** ${primaryLoc.region || 'N/A'}`, `**Country:** ${primaryLoc.country} :flag_${getCountryCode(primaryLoc.country)}:`, `**Timezone:** ${primaryLoc.tz_id}`, `**Local Time:** ${formatLocalTime(primaryLoc.localtime)}`].join('\n'),
    inline: false
  } as any);
  embed.addFields({
    name: '🗺️ Coordinates',
  ...
  if (compareData) {
    ...
    embed.addFields({
      name: `🏙️ ${compLoc.name}`,
      value: [`**Region:** ${compLoc.region || 'N/A'}`, `**Country:** ${compLoc.country} :flag_${getCountryCode(compLoc.country)}:`, `**Timezone:** ${compLoc.tz_id}`, `**Local Time:** ${formatLocalTime(compLoc.localtime)}`].join('\n'),
      inline: false
    } as any);

    // Distance and direction calculation
    const distance = calculateDistance(primaryLoc.lat, primaryLoc.lon, compLoc.lat, compLoc.lon);
    const direction = calculateDirection(primaryLoc.lat, primaryLoc.lon, compLoc.lat, compLoc.lon);
    embed.addFields({
  ...
  embed.setFooter({
    text: 'Geographic data powered by WeatherAPI'
  });
  return embed;
}

// Helper functions
function errorEmbed(message: any) {
  return new EmbedBuilder().setTitle('❌ Error').setDescription(message || 'An unknown error occurred').setColor('#E74C3C').setTimestamp();
}
function createProgressBar(current: any, max: any, length: number = 10) {
...
function getSeverityEmoji(severity: any) {
  const severityMap = {
    'extreme': '🔴',
    'severe': '🟠',
    'moderate': '🟡',
...
function getCountryCode(country: any) {
...
function getUTCOffset(timezone: any) {
  try {
    const now = new Date();
...
function calculateTimeDifference(time1: any, time2: any) {
...
function calculateDirection(lat1: any, lon1: any, lat2: any, lon2: any) {
  const dLon = (lon2 - lon1) * Math.PI / 180;
...
function calculateTimezoneOffset(tz1: any, tz2: any) {
  if (!tz1 || !tz2) return null;
  try {
    const date = new Date();
    const time1 = new Date(date.toLocaleString('en-US', {
      timeZone: tz1
    }));
    const time2 = new Date(date.toLocaleString('en-US', {
      timeZone: tz2
    }));
...
function buildPageEmbed(state: any) {
  switch (state.currentPage) {
    case 1:
      return buildCurrentWeatherEmbed(state);
    case 2:
      return buildForecastEmbed(state);
    case 3:
      return buildAstronomyEmbed(state);
    case 4:
      return buildAlertsEmbed(state);
    case 5:
      return buildLocationEmbed(state);
    default:
      return errorEmbed('Invalid page');
  }
}
export default {
  data: new SlashCommandBuilder().setName('weather').setDescription('Advanced weather information with beautiful visualizations').setIntegrationTypes(0, 1).setContexts(0, 1, 2).addStringOption((option: any) => option.setName('city').setDescription('Enter city name, postal code, IP address, or coordinates').setRequired(true).setAutocomplete(true)).addStringOption((option: any) => option.setName('compare').setDescription('Optional: Enter another location to compare').setRequired(false).setAutocomplete(true)).addStringOption((option: any) => option.setName('units').setDescription('Choose temperature units').setRequired(false).addChoices({
    name: 'Metric (°C)',
    value: 'metric'
  }, {
    name: 'Imperial (°F)',
  ...
  async autocomplete(interaction: any) {
    ...
    try {
      ...
      await interaction.respond(suggestions.map((city: any) => ({
        name: city,
        value: city
      })).slice(0, 25));
    } catch (error: any) {
  ...
  async execute(interaction: any) {
    ...
    try {
      ...
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`weather_prev_${stateId}`).setLabel('Previous').setEmoji('⬅️').setStyle(ButtonStyle.Primary).setDisabled(state.currentPage === 1), new ButtonBuilder().setCustomId(`weather_page_${stateId}`).setLabel(`Page ${state.currentPage}/${state.totalPages}`).setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId(`weather_next_${stateId}`).setLabel('Next').setEmoji('➡️').setStyle(ButtonStyle.Primary).setDisabled(state.currentPage === state.totalPages), new ButtonBuilder().setCustomId(`weather_refresh_${stateId}`).setLabel('Refresh').setEmoji('🔄').setStyle(ButtonStyle.Success));

      // Register button handlers
      registerButton(`weather_prev_${stateId}`, async (btnInt: any) => {
        try {
          ...
          const newRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`weather_prev_${stateId}`).setLabel('Previous').setEmoji('⬅️').setStyle(ButtonStyle.Primary).setDisabled(state.currentPage === 1), new ButtonBuilder().setCustomId(`weather_page_${stateId}`).setLabel(`Page ${state.currentPage}/${state.totalPages}`).setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId(`weather_next_${stateId}`).setLabel('Next').setEmoji('➡️').setStyle(ButtonStyle.Primary).setDisabled(state.currentPage === state.totalPages), new ButtonBuilder().setCustomId(`weather_refresh_${stateId}`).setLabel('Refresh').setEmoji('🔄').setStyle(ButtonStyle.Success));
      ...
      registerButton(`weather_next_${stateId}`, async (btnInt: any) => {
        try {
          if (btnInt.user.id !== interaction.user.id) {
            return await btnInt.reply({
              content: '❌ You cannot use these controls.',
      ...
      registerButton(`weather_refresh_${stateId}`, async (btnInt: any) => {
        try {
          if (btnInt.user.id !== interaction.user.id) {
            return await btnInt.reply({
              content: '❌ You cannot use these controls.',
          ...
          const loadingEmbed = new EmbedBuilder().setTitle('🔄 Refreshing Weather Data...').setDescription('Please wait while we fetch the latest information.').setColor('#3498DB');
          await btnInt.editReply({
            embeds: [loadingEmbed],
            components: []
          });
          ...
          const errorEmbed = new EmbedBuilder().setTitle('❌ Refresh Failed').setDescription(`Could not refresh weather data: ${error.message}`).setColor('#E74C3C');
          await btnInt.editReply({
            embeds: [errorEmbed],
            components: []
          });
      ...
      const errorEmbed = new EmbedBuilder().setTitle('❌ Weather Command Error').setDescription(`${error.message}`).setColor('#E74C3C').setTimestamp();
      await interaction.editReply({
        embeds: [errorEmbed],
        components: []
      });
