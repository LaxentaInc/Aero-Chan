  ...
  (async () => {
    try {
      ...
      for (let i = 0; i < hugGifCache.CACHE_SIZE; i++) {
        promises.push((axios.get('https://nekos.best/api/v2/hug', {
          timeout: 5000
...
const incrementHugCount = (userId1: any, userId2: any) => {
...
export default {
  data: new SlashCommandBuilder().setName('hug').setDescription('Hug someone warmly!').addUserOption((option: any) => option.setName('target').setDescription('The user you want to hug').setRequired(true)).addBooleanOption((option: any) => option.setName('ephemeral').setDescription('Whether to make the response visible only to you (default: false)')).setIntegrationTypes(0, 1) // guild and dm integrations
  .setContexts(0, 1, 2),
  // guild, dm, and voice contexts
  async execute(interaction: any) {
    ...
    try {
      ...
      const embed = new EmbedBuilder().setDescription(`-# **${interaction.user.username}** gives **${userToHug.username}** a warm hug!**\n` + `-# ***${interaction.user.username}** has hugged **${userToHug.username}** ${hugCount} ${hugCount === 1 ? 'time' : 'times'}.*`).setImage(hugGif);
