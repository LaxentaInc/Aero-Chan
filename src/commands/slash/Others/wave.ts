...
const saveStats = (s: any) => {
  try {
    ...
    console.error('failed to save wave stats:', e);
  }
};
...
export default {
  data: new SlashCommandBuilder().setName('wave').setDescription('Greeting! Wave! (●\'◡\'●)').setIntegrationTypes(0, 1).setContexts(0, 1, 2).addUserOption((option: any) => option.setName('target').setDescription('The user to wave at (optional)').setRequired(false)),
  async execute(interaction: any) {
    const targetUser = interaction.options.getUser('target');
    let components = [];
    if (targetUser && targetUser.id !== interaction.user.id) {
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('wave_back').setLabel('Wave Back').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('five_back').setLabel('High Five').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('smile_back').setLabel('Smile').setStyle(ButtonStyle.Secondary));
      components.push(row);
    }
    ...
    try {
      ...
      collector.on('collect', async (i: any) => {
        ...
        const responseEmbed = new EmbedBuilder().setDescription(`-# **${targetUser.username}** chose to **${action}** back at **${interaction.user.username}!**`).setImage(actionGifUrl);
