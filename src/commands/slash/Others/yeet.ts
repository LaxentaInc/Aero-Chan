...
const saveStats = (s: any) => {
  try {
    ...
    console.error('failed to save yeet stats:', e);
  }
};
...
export default {
  data: new SlashCommandBuilder().setName('yeet').setDescription('YEET someone into orbit!').setIntegrationTypes(0, 1).setContexts(0, 1, 2).addUserOption((option: any) => option.setName('target').setDescription('The user to yeet').setRequired(false)),
  async execute(interaction: any) {
    ...
    try {
      ...
      if (!gif) return interaction.editReply({
        content: 'couldn\'t yeet. try again!'
      });
      let desc;
      if (target && target.id !== interaction.user.id) {
        ...
        desc = `-# **${interaction.user.username}** yeets themselves!`;
      }
