...
export default {
  ...
  async execute(message: any, args: any) {
    if (!message.member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.reply('<:Lperms:1328691524245913680> You do not have permission to ban members! <a:FaceSlap:1327965185490550794>');
    }
    if (!message.guild.members.me.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.reply('<a:BlackHeart:1327965185490550794> I lack the necessary permissions to ban members! Please update my permissions.');
    }
    const userToBan = message.mentions.users.first();
    if (!userToBan) {
      return message.reply('<a:q:1327965185490550794> Please mention a user to ban! Example: !ban @username [reason]');
    }
    ...
    const confirmationEmbed = new EmbedBuilder().setColor(0xff4747).setTitle('<a:q:1327965185490550794> Ban Confirmation').setDescription(`Are you sure you want to ban <a:banned:1140133494199173121> **${userToBan.tag}**? Please confirm!`).addFields({
    ...
    const buttonRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`confirmBan-${message.id}`).setLabel('Confirm').setEmoji('<a:Checkmark:1327965185490550794>').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`cancelBan-${message.id}`).setLabel('Cancel').setEmoji('<a:cancel:1327965151781064715>').setStyle(ButtonStyle.Secondary));
    const confirmationMessage = await message.reply({
      embeds: [confirmationEmbed],
      components: [buttonRow]
    });
    const authorId = message.author.id;
    registerButton(`confirmBan-${message.id}`, [authorId], async (interaction: any) => {
      try {
    ...
    registerButton(`cancelBan-${message.id}`, [authorId], async (interaction: any) => {
      try {
        await deferSafe(interaction); // Safe deferral of interaction
        await editSafe(interaction, {
