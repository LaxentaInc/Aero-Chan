...
export default {
  ...
  async execute(message: any, args: any, client: any) {
    ...
    let ticketChannel = guild.channels.cache.find(channel => channel.name === `ticket-${user.username.toLowerCase()}`) as any;
    if (ticketChannel) {
      return message.reply(`You already have a ticket open: ${ticketChannel}`);
    }
    let category = guild.channels.cache.find(c => c.name === categoryName && c.type === 4) as any;
    if (!category) {
    ...
    const embed = new EmbedBuilder().setTitle('Support Ticket').setDescription('An admin will assist you shortly. Use the buttons below for actions.').setColor('Blue').setFooter({
      text: JSON.stringify({
        userId: user.id
      })
    });
    ...
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(claimButtonId).setLabel('Claim Ticket').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(closeButtonId).setLabel('Close Ticket').setStyle(ButtonStyle.Danger).setDisabled(true));
    await ticketChannel.send({
      content: `<@${user.id}>`,
      embeds: [embed],
      components: [buttons]
    });
    registerButton(claimButtonId, [], async (interaction: any) => {
    ...
    registerButton(closeButtonId, [user.id], async (interaction: any) => {
