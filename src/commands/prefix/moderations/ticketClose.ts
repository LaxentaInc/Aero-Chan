...
export default {
  ...
  async execute(message: any, args: any) {
    ...
    try {
      ...
      const confirmEmbed = new EmbedBuilder().setColor('#ff0000').setDescription('Are you sure you want to close this ticket?');
      const confirmRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('confirm-close').setLabel('Yes, close it').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('cancel-close').setLabel('Cancel').setStyle(ButtonStyle.Secondary));
      await message.reply({
        embeds: [confirmEmbed],
        components: [confirmRow]
      });
      const filter = (i: any) => i.customId === 'confirm-close' || i.customId === 'cancel-close';
