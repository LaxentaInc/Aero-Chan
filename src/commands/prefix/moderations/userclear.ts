  ...
  async execute(message: any, args: any) {
    ...
    try {
      ...
      while (allUserMessages.length < amount && totalFetched < maxTotalFetch) {
        ...
        const userMessages = messages.filter((msg: any) => msg.author.id === userId);
      ...
      allUserMessages.sort((a: any, b: any) => b.createdTimestamp - a.createdTimestamp);
      const messagesToProcess = allUserMessages.slice(0, amount);

      // Separate deletable and non-deletable messages
      const fourteenDaysAgo = Date.now() - 14 * 24 * 60 * 60 * 1000;
      const deletableMessages = messagesToProcess.filter((msg: any) => msg.createdTimestamp > fourteenDaysAgo);
      const tooOldMessages = messagesToProcess.filter((msg: any) => msg.createdTimestamp <= fourteenDaysAgo);
      ...
      if (deletableMessages.length === 1) {
        ...
        const bulkDeletableMessages = deletableMessages.filter((msg: any) => Date.now() - msg.createdTimestamp < 14 * 24 * 60 * 60 * 1000 - 60000 // 1 minute buffer
        );
          ...
          try {
            ...
            for (const msg of bulkDeletableMessages) {
              try {
                await msg.delete();
                deletedCount++;
                await new Promise((resolve: any) => setTimeout(resolve, 1000)); // Rate limit protection
              } catch (err: any) {
        ...
        const remainingMessages = deletableMessages.filter((msg: any) => !bulkDeletableMessages.includes(msg));
      ...
      // Create result embed
      const embed = new EmbedBuilder().setTitle('User Messages Purged').setColor(deletedCount > 0 ? 0x00FF00 : 0xFF9900).addFields([{
        name: 'Target User',
