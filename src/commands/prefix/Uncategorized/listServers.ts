    ...
    try {
      ...
      const serverListPromises = guilds.map(async (guild: any, index: any) => {
        ...
        try {
          ...
          const channel = guild.channels.cache.find((ch: any) => ch.isTextBased() && ch.permissionsFor(guild.members.me).has("CreateInstantInvite")) as any;
          if (channel) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const embed = new EmbedBuilder().setTitle(`📋 Server List (${i + 1}/${chunks.length})`).setDescription(chunk.join("\n\n")).setColor(0x5865F2).setFooter({
      ...
      const errorMessage = await message.reply("There was an error trying to list the servers. Please try again later.");
      setTimeout(() => {
        message.delete().catch(() => {});
