...
const FALLBACK_CHARACTERS = [{
  CharacterImage: "https://cdn.myanimelist.net/images/characters/2/376375.jpg",
  // Miku
  CharacterName: "Miku Nakano",
  AnimeName: "The Quintessential Quintuplets",
  CharacterJapaneseName: "中野 三玖"
}, {
  CharacterImage: "https://cdn.myanimelist.net/images/characters/13/328243.jpg",
  // Zero Two
  CharacterName: "Zero Two",
  AnimeName: "DARLING in the FRANXX",
  CharacterJapaneseName: "ゼロツー"
}, {
  CharacterImage: "https://cdn.myanimelist.net/images/characters/10/397368.jpg",
  // Mai Sakurajima
  ...
  try {
    ...
    if (useRareApi) {
      ...
      const {
        ...
        headers,
        timeout: 3000
      }) as any); // Slightly reduced timeout
...
function getRemainingRolls(userId: any) {
  const usage = usageMap.rolls.get(userId) as any || {
    count: 0
  };
  return RATE_LIMITS.rolls.max - usage.count;
}
function createWaifuEmbed(waifuData: any, cost: any, userId: any) {
  try {
    const remainingRolls = getRemainingRolls(userId);
    let embed = new EmbedBuilder().setImage(waifuData.image).setFooter({
      text: `${remainingRolls} remaining`
    });
    if (waifuData.isRare) {
      // Rare waifu from image APIs
      embed.setTitle('You stubmled upon an legendry waifu while advernturing!').setDescription('A mysterious waifu has been spotted! she\'s absolutely stunning! 💫').addFields({
      ...
      embed.setTitle(`${waifuData.characterName}`).setDescription(`**From:** ${waifuData.animeName}`).addFields([{
...
function checkRateLimit(type: any, userId: any) {
  const now = Date.now();
  ...
  const data = usage.get(userId) as any;
  ...
  if (data.count >= limit.max) {
    return {
      limited: true,
      reset: Math.floor((data.timestamp + limit.cooldown) / 1000)
    };
  }
  data.count++;
  return {
...
export default {
  ...
  async execute(message: any, args: any) {
    ...
    try {
      ...
      const marryBtn = new ButtonBuilder().setCustomId(`marry_${rollId}`).setEmoji('💖').setStyle('Primary');
      ...
      const filter = (i: any) => i.customId === `marry_${rollId}` && i.user.id === userId;
      ...
      collector.on('collect', async (interaction: any) => {
        try {
          ...
          const disabledRow = new ActionRowBuilder().addComponents(ButtonBuilder.from(marryBtn).setDisabled(true));
          ...
          const confirmBtn = new ButtonBuilder().setCustomId(`confirm_${rollId}`).setEmoji('<a:marryyy:1461768553513615515>').setStyle('Success').setLabel('Confirm');
          const cancelBtn = new ButtonBuilder().setCustomId(`cancel_${rollId}`).setStyle('Danger').setLabel('Cancel');
          ...
          const proposalEmbed = new EmbedBuilder().setDescription(waifuData.isRare ? `<a:marryyy:1461768553513615515> **Marriage Proposal for** <@${userId}>!\n\n✨ **Rare Mysterious Waifu** ✨\nCost: **${economy.formatCurrency(cost)}**\nYour Balance: **${economy.formatCurrency(balance)}**` : `<a:clapher:1461768816164999242> **Marriage Proposal for** <@${userId}>!\n\n**${waifuData.characterName}**\n**${waifuData.animeName}**\n\nCost: **${economy.formatCurrency(cost)}**\nYour Balance: **${economy.formatCurrency(balance)}**`);
          ...
          const confirmFilter = (i: any) => i.user.id === userId && (i.customId === `confirm_${rollId}` || i.customId === `cancel_${rollId}`);
          const confirmCollector = confirmationMsg.createMessageComponentCollector({
            filter: confirmFilter,
            time: 60000,
            max: 1
          });
          confirmCollector.on('collect', async (i: any) => {
            if (i.customId === `cancel_${rollId}`) {
              await i.update({
                content: `${getRandomElement(REACTIONS.failure)} Marriage cancelled.`,
                components: []
              });
              return;
            }
            ...
            try {
              ...
              if (currentBalance < cost) {
                await i.update({
                  content: `${getRandomElement(REACTIONS.failure)} Transaction failed: Insufficient funds.`,
                  components: []
                });
                return;
              }
              ...
              // Save to DB
              let record = await (Marriage.findOne({
                userId
              }) as any);
              if (!record) {
                record = new Marriage({
              ...
              await record.save();
              const marriageMessage = waifuData.isRare ? `${getRandomElement(REACTIONS.success)} **<@${userId}>**, You are lucky! <a:clapher:1461768816164999242> you married a rare waifu!\nCost: **${economy.formatCurrency(cost)}**\nNew balance: ${economy.formatCurrency(newBalance)}` : `${getRandomElement(REACTIONS.success)} **<@${userId}>**, you married **${waifuData.characterName}** from **${waifuData.animeName}**!\nCost: **${economy.formatCurrency(cost)}**\nNew balance: ${economy.formatCurrency(newBalance)}`;
              await i.update({
          ...
          await interaction.followUp({
            content: "Something went wrong during marriage.",
            ephemeral: true
          });
        }
      });
