...
export default {
  ...
  async execute(message: any) {
    ...
    const description = topUsers.map((user: any, index: any) => {
    ...
    const embed = new EmbedBuilder().setColor('#F1C40F').setTitle('<:leaderboard:1461766511629963284> Richest Users').setDescription(description).setFooter({
      text: 'Compete for the top spot bbg!'
    });
