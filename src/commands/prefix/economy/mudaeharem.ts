...
export default {
  ...
  async execute(message: any, args: any, client: any) {
    ...
    const marriageData = await (Marriage.findOne({
      userId
    }) as any);
    ...
    function generateEmbed(page: any) {
      const waifu = waifus[page];
      const costText = typeof waifu.cost === 'number' ? waifu.cost.toLocaleString() : "N/A";
      let embed = new EmbedBuilder().setTitle(`${message.author.username}'s Harem <a:marryyy:1461768553513615515>`).setDescription(`Displaying waifu ${page + 1} of ${waifus.length}\n<:consolation:1461662922496544820> **Spent in marriages:** ⏣ ${totalSpent.toLocaleString()}`).setImage(waifu.url).setFooter({
        text: `Page ${page + 1} / ${waifus.length} • Married ${new Date(waifu.marriedAt).toLocaleDateString()}`
      });
      ...
      if (waifu.isRare || !waifu.characterName && !waifu.animeName) {
        ...
        embed.setColor('#FFD700') // Gold for rare
        .addFields([{
    ...
    function generateActionRow(page: any) {
      ...
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(prevCustomId).setLabel('Previous')
      // .setEmoji('◀️')
      .setStyle('Primary').setDisabled(prevDisabled), new ButtonBuilder().setCustomId(statsCustomId).setLabel('Stats').setEmoji('<a:st:1461771374799622183>').setStyle('Secondary'), new ButtonBuilder().setCustomId(nextCustomId).setLabel('Next')
      // .setEmoji('▶️')
      .setStyle('Primary').setDisabled(nextDisabled));
      return {
        row,
        prevCustomId,
        nextCustomId,
    ...
    function generateStatsEmbed() {
      ...
      const embed = new EmbedBuilder().setTitle(`${message.author.username}'s Harem Statistics 📊`)
      // .setColor('#9932CC')
      .setThumbnail(message.author.displayAvatarURL()).addFields([{
    ...
    registerButton(prevCustomId, [userId], async (interaction: any) => {
    ...
    registerButton(nextCustomId, [userId], async (interaction: any) => {
      await deferSafe(interaction);
    ...
    registerButton(statsCustomId, [userId], async (interaction: any) => {
      await deferSafe(interaction);
      const statsEmbed = generateStatsEmbed();

      // Back button to return to harem view
      const backButton = new ButtonBuilder().setCustomId(`harem_back_${message.id}`).setLabel('Back to Harem').setEmoji('<a:marker_1326464173361856524:1342443432240746577>').setStyle('Secondary');
      ...
      registerButton(`harem_back_${message.id}`, [userId], async (backInteraction: any) => {
        ...
        try {
          await editSafe(backInteraction, {
            embeds: [newEmbed],
            components: [newRow]
          });
        } catch (error: any) {
