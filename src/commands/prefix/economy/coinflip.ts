...
export default {
  ...
  async execute(message: any, args: any) {
    ...
    const embed = new EmbedBuilder().setDescription(`<a:coinflip_main:1461659736536514797> **Coinflip Result**\n\n` + `**Your Choice:** ${choiceText}\n` + `**Result:** ${resultText}\n` + `**Bet:** ${economy.formatCurrency(bet)}\n` + `**Outcome:** ${isWin ? '<:y_win:1461659408420306955> You Win!' : 'You Lose!'}\n` + `**Winnings:** ${economy.formatCurrency(winnings)}\n` + `**Net:** ${net >= 0 ? '+' : ''}${economy.formatCurrency(net)}\n\n` + `**New Balance:** ${economy.formatCurrency(balance)}`).setFooter({
      text: 'Usage: !cf <amount> <h/t> • h = heads, t = tails'
    });
