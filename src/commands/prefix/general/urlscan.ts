...
export default {
  ...
  async execute(message: any, args: any) {
    ...
    const statusMsg = await message.reply({
      embeds: [new EmbedBuilder().setTitle('🔄 Advanced URL Security Analysis').setDescription(`🔍 **initiating security scan**\n\n🔗 **target:** \`${url}\`\n👤 **requested by:** ${message.author.tag}`).setColor('#5865F2').setTimestamp()]
    });
    try {
      ...
      if (!scanId) {
        return statusMsg.edit({
          embeds: [new EmbedBuilder().setDescription('❌ failed to submit scan.').setColor(0xff4747)]
        });
      }

      // step 2: update status
      await statusMsg.edit({
        embeds: [new EmbedBuilder().setTitle('🔍 Advanced URL Security Analysis').setDescription(`📋 **scan ID:** \`${scanId}\`\n⏱️ **status:** analysis in progress...\n🔍 **type:** deep security scan`).setColor('#5865F2').setTimestamp()]
      }).catch(() => {});
      ...
      for (let attempt = 0; attempt < 12; attempt++) {
        ...
        // progress update
        await statusMsg.edit({
          embeds: [new EmbedBuilder().setTitle('🔍 Advanced URL Security Analysis').setDescription(`📋 **scan ID:** \`${scanId}\`\n⏱️ **elapsed:** ${(attempt + 1) * 5} seconds\n🔍 analyzing page content...`).setColor('#5865F2').setTimestamp()]
        }).catch(() => {});
      }
      if (!result) {
        return statusMsg.edit({
      ...
      const embed = new EmbedBuilder().setTitle('🖥️ Security Scan Complete').setURL(scan.page?.url || scan.task?.url || url).setColor(verdict.color).setTimestamp();
      ...
      if (scan.data?.performance) {
        ...
        const fmtMs = (ms: any) => ms == null || isNaN(ms) ? 'n/a' : `${Math.round(ms)}ms`;
        embed.addFields({
          name: '⚡ Performance',
      ...
      if (scan.meta?.processors?.domainCategories?.data) {
        ...
        const catStr = cats.map((c: any) => typeof c === 'string' ? c : c.name || 'unknown').join(', ');
        if (catStr) embed.addFields({
          name: '📂 Categories',
          value: catStr,
          inline: false
        } as any);
      ...
      if (scan.lists?.domains?.length > 0) {
        const domains = scan.lists.domains.slice(0, 8).map((d: any) => {
      ...
      embed.setFooter({
        text: `scan ID: ${scanId} • unlisted • powered by cloudflare`
      });
      ...
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel('Full Report').setURL(`https://radar.cloudflare.com/scan/${scanId}`).setStyle(ButtonStyle.Link).setEmoji('📊'), new ButtonBuilder().setLabel('Screenshot').setURL(screenshotUrl).setStyle(ButtonStyle.Link).setEmoji('📸'), new ButtonBuilder().setLabel('Mobile View').setURL(`https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/urlscanner/v2/screenshots/${scanId}.png?resolution=mobile`).setStyle(ButtonStyle.Link).setEmoji('📱'));
      await statusMsg.edit({
        embeds: [embed],
        components: [row]
      });
      ...
      if (error.response?.status === 429) errMsg = 'rate limited! try again in a minute.';else if (error.response?.status === 400) errMsg = 'invalid URL or scan request.';
      await statusMsg.edit({
        embeds: [new EmbedBuilder().setTitle('❌ Scan Failed').setDescription(errMsg).setColor('#F92F60').addFields({
          name: '💡 troubleshooting',
          value: '• ensure URL is valid\n• check if the site is publicly accessible\n• try just the domain without paths\n• some sites block automated scans'
        } as any).setTimestamp()]
      }).catch(() => {});
    }
  }
}; // verdict logic matching the slash command exactly
function getVerdict(verdicts: any) {
  if (!verdicts?.overall) return {
    style: '❓ **UNKNOWN**',
    description: 'could not determine verdict',
    color: '#808080'
  };
  if (verdicts.overall.malicious) return {
    style: '🚨 **MALICIOUS**',
    description: 'confirmed threats detected — do NOT visit this site',
    color: '#FF0000'
  };
  if (verdicts.overall.phishing) return {
    style: '🎣 **PHISHING**',
    description: 'this site is impersonating a legitimate service',
    color: '#FF0000'
  };
  if (verdicts.overall.categories?.includes('phishing')) return {
    style: '🎣 **PHISHING**',
    description: 'phishing activity detected',
    color: '#FF0000'
  };
  if (verdicts.overall.categories?.includes('malware')) return {
    style: '🚨 **MALWARE**',
    description: 'malware distribution detected',
    color: '#FF0000'
  };
  if (verdicts.overall.suspicious) return {
    style: '⚠️ **SUSPICIOUS**',
    description: 'potential risks found — proceed with caution',
    color: '#FFA500'
  };
  return {
