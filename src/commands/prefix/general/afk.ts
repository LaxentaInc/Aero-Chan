    ...
    try {
      ...
      if (existingAfk) {
        return message.reply({
          embeds: [new EmbedBuilder().setColor(0xFF0000).setDescription("<a:kittycat:1333358006720794624> HEY goofy! I thought YOU ARE ALREADY AFK!")]
        });
      }
      ...
      const member = message.guild.members.cache.get(userId) as any;
      ...
      message.reply({
        embeds: [new EmbedBuilder().setColor(0x0000FF).setDescription(`<a:kittycat:1333358006720794624> ${message.author} is now AFK: **${reason}**`)]
      });
      ...
      message.reply({
        embeds: [new EmbedBuilder().setColor(0xFF0000).setDescription("❌ An error occurred while setting your AFK status.")]
      });
    }
  }
};