...
function generateCompatibilityScore(name1: any, name2: any) {
  let score = 0;
...
function mergeNames(name1: any, name2: any) {
...
export default {
  ...
  async execute(message: any, args: any) {
    try {
      ...
      const embed = new EmbedBuilder().setColor('#ff69b4') // Pink color
      .setTitle(' <a:hmm:1138688928216522832> Matching!! ').addFields({
        name: '<a:b:1140133458606309386>',
        value: `**${firstPart}**`,
        inline: true
      } as any, {
        name: '<a:a:1140133458606309386>',
        value: `**${secondPart}**`,
        inline: true
      } as any, {
        name: '<:hmm:1260496728973643776>',
        value: mergedName
      } as any, {
        name: 'Compatibility',
        value: `${compatibility}% ${progressBar} ${description}`
      } as any).setFooter({
