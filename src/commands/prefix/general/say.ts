  ...
  async execute(message: any, args: any) {
    try {
      ...
      if (!message.member.permissions.has("ADMINISTRATOR")) {
        logger.warn(`User ${message.author.tag} tried to use the say command without sufficient permissions in ${message.guild.name}.`);
      ...
      message.channel.send("<a:block:1084349801346367518> There was an error executing the say command.").catch((err: any) => logger.error(`Failed to send error message: ${err.message}`));
    }
  }
};