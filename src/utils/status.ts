...
function updateBotStatus(client: any, lavalinkManager: any) {
  let i = 0;
  ...
  const updateStatus = async () => {
    try {
      ...
      const userCount = client.guilds?.cache?.reduce((acc: any, g: any) => {
  ...
  function getActivityTypeString(type: any) {
    const types = {
      0: "Playing",
      1: "Streaming",
      2: "Listening",
