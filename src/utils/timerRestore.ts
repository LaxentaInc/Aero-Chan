...
const Timer = (mongoose.models.Timer || mongoose.model('Timer', TimerSchema)) as any;
async function restoreTimers(client: any) {
  try {
    const activeTimers = await (Timer.find({}) as any);
    let restored = 0;
    let cleaned = 0;
...
async function handleTimerExpiration(client: any, timer: any, wasOffline: boolean = false) {
  const reminderEmbed = new EmbedBuilder().setColor('#57F287').setTitle(wasOffline ? '<a:hangingstarts13:1333359147655106581> Time\'s Up! (Missed while offline)' : '<a:hangingstarts13:1333359147655106581> Time\'s Up!').setDescription(timer.reason || 'Your timer has finished!').setTimestamp();
  let dmSent = false;
  try {
