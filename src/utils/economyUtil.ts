...
async function ensureUserCached(userId: any) {
  let user = userCache.get(userId) as any;
  if (user) return user;
...
async function getBalance(userId: any) {
...
async function setBalance(userId: any, newBalance: any) {
  const user = await ensureUserCached(userId);
...
async function claimDaily(userId: any, rewardAmount: number = 10000) {
  const user = await ensureUserCached(userId);
  const now = Date.now();
...
async function performWork(userId: any, minReward: number = 1000, maxReward: number = 5000) {
  const user = await ensureUserCached(userId);
  const now = Date.now();
...
async function transfer(fromId: any, toId: any, amount: any) {
...
async function flushDirtyData() {
  ...
  for (const userId of dirtyUsers) {
    const user = userCache.get(userId) as any;
  ...
  if (ops.length > 0) {
    ...
    try {
      ...
      idsToClear.forEach((id: any) => dirtyUsers.delete(id));
...
async function getLeaderboard(limit: number = 10) {
  ...
  const database = await connectDB();
  const collection = database.collection("users");
  return (collection.find() as any).sort({
    balance: -1
  }).limit(limit).toArray();
}
...
export { connectDB, getUser, getBalance, updateBalance, setBalance, claimDaily, performWork, transfer, getLeaderboard, formatCurrency, flushDirtyData as forceSave, DEFAULT_BALANCE };
export default {
  connectDB,
  getUser,
  getBalance,
