...
import AccountAgeProtectionModule from "../modules/ageVerify";
import botProtection from "../modules/bot-protection";
    ...
    try {
      ...
      if (member.user.bot) {
        ...
        try {
          ...
          const inviteLog = auditLogs.entries.find(entry => entry.target?.id === member.user.id && Date.now() - entry.createdTimestamp < 15000 // 15 seconds
          ) as any;
          if (inviteLog) {
