...
class AccountAgeProtectionModule {
  ...
  async syncConfigs() {
    ...
    try {
      const dbConfigs = await (this.collection.find({}) as any).toArray();
  ...
  getConfig(guildId: any) {
    ...
    const defaults = {
      ...
      minAccountAge: 7,
      // Minimum account age in days (default: 7 days)
      action: 'none',
      // Default action: 'none', 'kick', 'ban', 'timeout'
      timeoutDuration: 600,
      // Timeout duration in seconds (10 minutes)
      logChannelId: null,
      // Discord channel ID for notifications (null = system channel)
      trustedUsers: [],
      // User IDs that bypass age check
      trustedRoles: [],
      // Role IDs that bypass age check
      bypassTrusted: true,
      // Whether trusted users bypass the check
      logActions: true,
      // Log actions to configured channel
   ...
   */
  async handleMemberJoin(member: any) {
    // Skip bots
   ...
   */
  async shouldTakeAction(member: any, userData: any, config: any) {
   ...
   */
  async handleYoungAccount(member: any, actionData: any, config: any) {
    ...
    try {
      ...
      const alertEmbed = new EmbedBuilder().setColor(actionColors[config.action] || 0xF1C40F).setTitle(`<:warning:1422451081224392816> Young Account Detected`).setThumbnail(user.displayAvatarURL({
        dynamic: true,
        size: 256
      })).setDescription(`**${user.username}** joined with a suspiciously young account.`).addFields({
        name: '<:timeout:1422451090259181568> User',
        value: `${user} (\`${user.id}\`)`,
        inline: true
      } as any, {
        name: '📅 Account Age',
        value: `**${actionData.accountAgeDays}** days (min: ${config.minAccountAge})`,
        inline: true
      } as any, {
        name: '📆 Created',
        ...
        name: '🖼️ Avatar',
        value: actionData.analysis?.hasAvatar ? 'Yes' : 'No',
        inline: true
      } as any, ...(config.action === 'timeout' ? [{
        name: '⏰ Timeout Duration',
        value: `${config.timeoutDuration}s`,
        inline: true
      }] : [])).setFooter({
        text: 'Account Age Verification | /dashboard to configure'
      }).setTimestamp();
  ...
  isTrustedUser(user: any, guild: any, config: any) {
    ...
    if (member && config.trustedRoles.some((roleId: any) => member.roles.cache.has(roleId))) {
      return true;
    }
    return false;
  }
   ...
   */
  async notifyAdmins(guild: any, alertEmbed: any) {
    try {
   ...
   */
  async notifyOwner(guild: any, user: any, actionData: any, config: any, actionResult: any, alertEmbed: any) {
    try {
      const owner = await guild.fetchOwner();
      if (!owner) return;

      // build a dm-specific embed based on the alert embed
      const dmEmbed = new EmbedBuilder(alertEmbed.data).setTitle(`<:warning:1422451081224392816> Young Account Alert — ${guild.name}`).setDescription(`A young account was detected in **${guild.name}**.`).setFooter({
        text: 'Account Age Verification | Configure via /dashboard'
      });
      await owner.send({
        embeds: [dmEmbed]
      }).catch(() => {});
  ...
  async createDefaultConfig(guildId: any) {
    ...
    const defaultConfig = {
      ...
      // Module enabled by default
      minAccountAge: 7,
      // 7 days minimum account age
      action: 'none',
      // Default to just notify
      timeoutDuration: 600,
      // 10 minutes timeout (if using timeout action)
      logChannelId: null,
      // null = use system channel
      trustedUsers: [],
      // User IDs that bypass age check
      trustedRoles: [],
      // Role IDs that bypass age check
      bypassTrusted: true,
      // Whether trusted users bypass check
      logActions: true,
      // Log actions to channel
      debug: true // Debug logging
    };
    try {
...
export default new AccountAgeProtectionModule();