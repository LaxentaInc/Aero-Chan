...
import { saveButtons, loadButtons, loadAllButtons, persistButtons, BUTTON_EXPIRY_MS } from "./storage";
import { restoreUserRoles } from "./punishment";
import { restoreRolePermissions } from "./neutralize";
import { isTrusted } from "./detection";
/**
 ...
 */
async function registerStoredButtons(client: any) {
  try {
 ...
 */
function buildOwnerActionRow(executor: any, roleId: any, wasRoleNeutralized: any) {
  ...
  const buttons = [new ButtonBuilder().setCustomId(whitelistId).setLabel('Whitelist User').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(restoreUserRolesId).setLabel('Restore User Roles').setStyle(ButtonStyle.Primary)];
  if (wasRoleNeutralized) {
    buttons.push(new ButtonBuilder().setCustomId(restoreRolePermsId).setLabel('Restore Role Perms').setStyle(ButtonStyle.Secondary));
  }
  return {
 ...
 * - Ban
 */
function buildWhitelistedActionRow(executor: any) {
  ...
  const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(unWhitelistId).setLabel('Unwhitelist').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(stripId).setLabel('Strip Roles').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(kickId).setLabel('Kick').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(banId).setLabel('Ban').setStyle(ButtonStyle.Danger));
  return {
    row,
    unWhitelistId,
    stripId,
 ...
 */
async function registerAndPersistButtons(guildId: any, buttonMetas: any) {
 ...
 */
async function handleButtonAction(interaction: any, meta: any) {
  if (!meta) return;
  ...
  if (!guild && metaGuildId) {
    guild = client.guilds.cache.get(metaGuildId) as any || (await client.guilds.fetch(metaGuildId).catch(() => null));
  }
  if (!guild) {
  ...
  const currentConfig = apaInstance?.getConfig?.(guildId) || {
    trustedUsers: [],
    trustedRoles: [],
    whitelistedUsers: []
  };
  ...
  let executorMember = interaction.member || guild.members.cache.get(interaction.user.id) as any;
  if (!executorMember) {
  ...
  try {
    ...
    if (actionType === 'whitelist') {
      ...
      const list = Array.isArray(currentConfig.whitelistedUsers) ? currentConfig.whitelistedUsers : [];
      const filtered = list.filter((id: any) => id !== targetUserId);
      const newConfig = {
        ...currentConfig,
        whitelistedUsers: filtered
      };
      if (apaInstance?.updateConfig) {
        await apaInstance.updateConfig(guildId, newConfig);
      }
      ...
      await interaction.reply({
        content: result.success ? `🛡️ Stripped **${result.count}** dangerous role(s) from ${targetUser?.username || targetUserId}.` : `⚠️ Could not strip roles: ${result.reason}`,
        ephemeral: true
      }).catch(() => {});
      ...
      await targetMember.ban({
        reason: 'APA: Owner action via button'
      });
      await interaction.reply({
...
export { buildOwnerActionRow, buildWhitelistedActionRow, registerStoredButtons, registerAndPersistButtons, handleButtonAction };
export default {
  buildOwnerActionRow,
