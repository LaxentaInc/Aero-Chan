...
async function stripDangerousRoles(guild: Guild, executor: User, prefetchedMember = null) {
  try {
    ...
    const dangerousRoles = member.roles.cache.filter((role: any) => role.id !== guild.id &&
    // Not @everyone
    !role.managed &&
    // Not bot/integration managed
    botMember.roles.highest.position > role.position &&
    // Bot can remove it
    DANGEROUS_PERMISSIONS.some((perm: any) => role.permissions.has(perm)));
    ...
    const strippedRoles = dangerousRoles.map((role: any) => ({
      id: role.id,
      name: role.name,
 ...
 */
async function executePunishment(member: any, guild: any, config: any) {
  try {
    if (!member) {
      return {
        success: false,
    ...
    if (action === 'ban') {
      ...
      return {
        success: true,
        action: `Timed out for ${config.timeoutMinutes || 30} minutes`
      };
    }
    return {
      success: false,
 ...
 */
async function restoreUserRoles(guild: any, userId: any) {
  try {
    ...
    if (!member) {
      return {
        success: false,
        restoredCount: 0,
        errors: ['User is no longer in the server']
      };
    }
    ...
    if (!snapshot || !Array.isArray(snapshot.strippedRoles) || snapshot.strippedRoles.length === 0) {
      return {
        success: false,
        restoredCount: 0,
        errors: ['No stored strip data for this user']
      };
    }
    let restoredCount = 0;
    const errors = [];
    ...
    for (const roleData of snapshot.strippedRoles) {
      const role = guild.roles.cache.get(roleData.id) as any;
      if (!role) {
 ...
 */
function getActionsTakenMessage(results: any) {
  const lines = [];
...
export { stripDangerousRoles, executePunishment, restoreUserRoles, getActionsTakenMessage };
export default {
  stripDangerousRoles,
  executePunishment,
  restoreUserRoles,
