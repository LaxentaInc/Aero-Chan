...
import { sendPunishmentDM } from "./notification";
/**
 * Punishment Logic
 */
 ...
 */
function buildPunishmentReason(violations: any) {
  const reasons = violations.map((v: any) => {
    switch (v.type) {
      case 'message_spam':
 ...
 * Execute punishment
 */
async function executePunishment(context: any, targetMember: any, violations: any, config: any, stats: any, notifyPunishmentFn: any) {
  try {
 ...
 */
async function executeWarningTimeout(member: any, reason: any) {
  try {
...
export { buildPunishmentReason, executePunishment, executeWarningTimeout };
export default {
