...
import { restoreUserPermissions } from "./punishment";
 ...
 */
function buildOwnerActionRow(executor: any) {
  const userId = executor.id;
  const whitelistId = `antinuke_whitelist_${userId}`;
  const restoreId = `antinuke_restore_${userId}`;
  const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(whitelistId).setLabel('Whitelist User').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(restoreId).setLabel('Restore Permissions').setStyle(ButtonStyle.Primary));
  return {
    row,
    whitelistId,
    restoreId
...
async function handleButtonAction(interaction: any, meta: any) {
  ...
  const guild = interaction.guild || (metaGuildId ? client.guilds.cache.get(metaGuildId) as any : null);
  if (!guild) {
  ...
  try {
    if (actionType === 'whitelist') {
      ...
      await targetMember.ban({
        reason: 'AntiNuke owner action (ban via button)'
      });
      await interaction.reply({
...
export { buildOwnerActionRow, buildWhitelistedActionRow, persistButtons, registerStoredButtons, registerAndPersistButtons };
export default {
  buildOwnerActionRow,
