...
async function findExecutor(guild: any, eventType: any, target: any, context: any) {
  ...
  const cached = context.executorCache.get(cacheKey) as any;
  ...
  if (context.auditLogRequests.has(cacheKey)) {
    ...
    return await (context.auditLogRequests.get(cacheKey) as any);
  }
...
function isTrusted(user: any, guild: any, config: any) {
  ...
  return member?.roles.cache.some((r: any) => config.trustedRoles?.includes(r.id)) || false;
}
...
async function canPunish(guild: any, executorId: any) {
  try {
    ...
    if (!member.manageable) {
      return {
        can: false,
        reason: 'Role hierarchy: Target has higher/equal role than bot'
      };
    }
...
export { getAuditType, findExecutor, isTrusted, getThreshold, canPunish };
export default {
  getAuditType,
  findExecutor,
  isTrusted,
