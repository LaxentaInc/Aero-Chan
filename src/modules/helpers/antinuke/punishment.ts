...
const DANGEROUS_PERMISSIONS = ['Administrator', 'ManageGuild', 'ManageRoles', 'ManageChannels', 'BanMembers', 'KickMembers'];
...
async function restoreUserPermissions(guild: any, userId: any) {
  try {
    ...
    const rolesToAdd = [];
    for (const roleData of snapshot.strippedRoles.filter((r: any) => !r.managed)) {
...
export { stripDangerousRoles, executePunishment, getActionsTakenMessage, DANGEROUS_PERMISSIONS, restoreUserPermissions };
export default {
  stripDangerousRoles,
  executePunishment,
