...
async function notifyOwner(guild: any, executor: any, type: any, config: any, actionCount: any, actionType = null, extra: Record<string, any> = {}) {
  ...
  try {
    const {
      ...
      buttonMetas,
      isBot
    } = await prepareAntiNukePayload(guild, executor, type, config, actionCount, actionType, extra);
...
export { notifyOwner, logToChannel, getActionDescription };
export default {
  notifyOwner,
  logToChannel,
