 ...
 */
async function handleWhitelist(interaction: any, meta: any, botProtection: any, currentConfig: any) {
  ...
  if (currentConfig.whitlistedBots && currentConfig.whitlistedBots.includes(targetId)) {
    return interaction.reply({
      content: '✅ Bot is already whitelisted.',
      ephemeral: true
    });
  }
  ...
  await botProtection.updateConfig(meta.guildId, {
  ...
  try {
    ...
    const newRows = msg.components.map((row: any) => {
 ...
 */
async function handleRestorePerms(interaction: any, meta: any, botProtection: any, currentConfig: any) {
  ...
  if (!strippedPermissions) {
    return interaction.reply({
      content: '❌ Error: Missing permission data in button metadata.',
      ephemeral: true
    });
  }
  const {
  ...
  const guild = interaction.guild || interaction.client.guilds.cache.get(meta.guildId) as any;
  ...
  if (!member) {
    return interaction.reply({
      content: '❌ Bot is no longer in the server.',
      ephemeral: true
    });
  }
  ...
  if (result.success) {
    ...
    const successEmbedUpdate = (oldEmbed: any) => {
    ...
    const successRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('restored_done').setLabel(`Restored by ${restorer.username}`).setStyle(ButtonStyle.Success).setEmoji(EMOJIS.VERIFIED || '✅').setDisabled(true));
    ...
    if (!relatedMessages.some((m: any) => m.messageId === interaction.message.id)) {
      updatePromises.push(interaction.message.edit({
        embeds: [successEmbedUpdate(interaction.message.embeds[0])],
        components: [successRow]
      }).catch((err: any) => console.log(`${LOG_PREFIX} Failed to update interaction message: ${err.message}`)));
    }
