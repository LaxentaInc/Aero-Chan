...
class BotProtectionModule {
  ...
  async handleBotJoin(member: any, inviter = null) {
    ...
    const botData = {
      ...
      botAgeHours: (Date.now() - member.user.createdTimestamp) / (1000 * 60 * 60),
      // Age in hours
  ...
  async handleSuspiciousBot(member: any, inviter: any, suspicionData: any, config: any, strippedPermissions: any) {
    ...
    logManager.log(guild, 'SUSPICIOUS_BOT', {
      ...
      fields: [{
        ...
        name: '👤 Added By',
        value: inviter ? `${inviter.username} (${inviter.id})` : 'Unknown',
        inline: true
      }, {
        name: '⚠️ Reasons',
        value: suspicionData.reasons.join(', ') || 'N/A',
        inline: false
      }]
    });
    const actionsTaken = {
    ...
    for (const action of config.punishmentActions) {
      try {
        if (action === 'kick_bot') {
          ...
          await member.ban({
            reason: `Suspicious bot - ${suspicionData.reasons.join(', ')}`,
...
export default new BotProtectionModule();