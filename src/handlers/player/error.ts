 ...
 */
function createSafeErrorEmbed(title: any, description: any) {
  return new EmbedBuilder().setAuthor({
    name: `❌ ${title}`,
    iconURL: CUSTOM_ICON
 ...
 */
function createPrivatePlaylistErrorEmbed(user: any) {
  return new EmbedBuilder().setAuthor({
    name: 'Private Playlist Detected',
    iconURL: CUSTOM_ICON
  }).setDescription(`**Hey ${user}, are you dumb?**\n\nThis playlist is **PRIVATE**. I can't access private playlists because, you know, they're **PRIVATE**.\n\n**Solution:**\n- Go to Spotify and make the playlist **PUBLIC**\n- Right-click playlist → Make public\n- Then share the link again\n\nOr, you know, just keep it private and expect magic to happen. Your choice.`).setFooter({
    text: 'Private ≠ Public • Amazing discovery, right?'
  }).setTimestamp();
}
export { createDumbErrorEmbed, createDomainErrorEmbed, createSafeErrorEmbed, createPrivatePlaylistErrorEmbed };
export default {
