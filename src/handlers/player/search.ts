 ...
 */
function normalizeTrack(track: any, requester: any) {
  return {
 ...
 */
async function searchTrack(client: any, query: any, requester: any, source: string = 'spotify') {
  console.log(`\n🔍 Searching for: "${query}" (source: ${source})`);
  try {
    let searchQuery;

    // Check if it's a direct URL
    if (query.includes('spotify.com') || query.includes('youtube.com') || query.includes('youtu.be') || query.includes('soundcloud.com')) {
      searchQuery = query;
    } else {
      const prefixes = {
        spotify: 'spsearch:',
    ...
    const normalizedTracks = result.tracks.map((track: any) => normalizeTrack(track, requester));
    return {
      data: normalizedTracks,
      loadType: result.loadType || 'search'
    };
  } catch (error: any) {
 ...
 */
async function searchTrackAlternative(client: any, query: any, requester: any, source: string = 'spotify') {
 ...
 */
async function smartSearch(client: any, query: any, requester: any) {
...
export { normalizeTrack, searchTrack, searchTrackAlternative, smartSearch };
export default {
  normalizeTrack,
  searchTrack,
