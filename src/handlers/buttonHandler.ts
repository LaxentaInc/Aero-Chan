import { Collection, ActionRowBuilder, ButtonBuilder, MessageFlags, ChannelType } from "discord.js";
...
class DMChannelUtils {
  static async safeMessageEdit(message: any, data: any) {
    try {
  ...
  static async safeMessageDelete(message: any) {
    try {
 ...
 */
async function registerButton(customId: any, authorizedIds: any[] = [], callback: any, options: Record<string, any> = {}) {
 ...
 */
async function updateMessageButtons(message: any, targetCustomId: any) {
  try {
    ...
    if (message.channel?.type === ChannelType.DM) {
      ...
      if (message.interaction?.webhook) {
        ...
        await message.interaction?.update({
          components: updatedComponents
        }).catch(() => {
 ...
 */
async function handleExpiredButton(interaction: any) {
  try {
...
async function handleButton(interaction: any) {
  ...
  try {
    interactionDoc = await (Interaction.findOne({
      customId
    }) as any);
  } catch (dbError: any) {
  ...
  const handlerData = handlers.get(customId) as any;
  if (!handlerData) {
    await handleExpiredButton(interaction);
    return;
  }
...
async function reloadHandlers() {
  try {
    const interactions = await (Interaction.find({}) as any);
...
reloadHandlers();
export { registerButton, handleButton, handlers, Interaction };
export default {
  registerButton,
  handleButton,
  handlers,
